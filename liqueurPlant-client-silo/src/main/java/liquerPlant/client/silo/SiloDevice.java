...
public class SiloDevice {
    ...
    /*
     * 		args=	{endpointName "softSim" lowLevelSensorHeight highLevelSensorHeight time2Fill-10%(ms) hasMixer hasHeater clientIP clientPort serverIP serverPort... }
     *  OR 	args=	{endpointName "hardSim" clientIP clientPort serverIP serverPort... }
     *	OR	args=	{endPointName "codesysController" inPipeName outPipeName sendInterval receiveInterval checkForChangesInterval clientIP clientPort serverIP serverPort...}
     *					0				1					2			3			4				5				6				7			8
     *
    ...
    */
    ...
    public SiloDevice(String[] args) {
    	...
    	int i=0; //position in which clientIP argument is.
    	if(args[1].equals("softSim")){
        	siloDriver=new SiloSimulatorDriver(new SiloParameters(Integer.parseInt(args[2]),Integer.parseInt(args[3]),Long.parseLong(args[4]),Boolean.parseBoolean(args[5]),Boolean.parseBoolean(args[6])) , endpointName);
        	i=7;
    	}
    	else if(args[1].equals("hardSim")){
    		siloDriver=new SiloHardwareDriver();
    		i=2;
    	}
    	else if(args[1].equals("codesysController")){
    		File inPipe=new File(args[2]);
    		File outPipe=new File(args[3]);
    		siloDriver=new SiloCodesysDriver(inPipe,outPipe,Integer.parseInt(args[4]),Integer.parseInt(args[5]),Integer.parseInt(args[6]));
    		i=7;
    	}
